...
class BnBMethod(val A: DenseMatrix[Double],
  ...
  private val r0 = if(c.forall(_ >= 0) && dl.forall(_ >= 0)) 0 else Double.MinValue
  private val (mu0, mu) = (0, DenseVector.zeros[Int](n))


  def solve: Either[String, DenseVector[Int]] = {
    val dsm: DualSimplexMethod = new DualSimplexMethod(A.copy, b.copy, c.copy, dl.copy, dh.copy)
    iteration(Seq(dsm), r0, mu0, mu)
  }

  @tailrec
  private def iteration(tasks: Seq[DualSimplexMethod], r0: Double, mu0: Int, mu: DenseVector[Int]): Either[String, DenseVector[Int]] = {
    if (tasks.isEmpty) {
      mu0 match {
          ...
          case false =>
            ...
            notInt match {
              case Some(value) =>
                ...
                val subtask1 = new DualSimplexMethod(currentTask.A.copy, currentTask.b.copy, currentTask.c.copy, currentTask.dl.copy, currentDh)
                currentDl(value._1) = math.ceil(value._2)
                val subtask2 = new DualSimplexMethod(currentTask.A.copy, currentTask.b.copy, currentTask.c.copy, currentDl, currentTask.dh.copy)
                subtask1.show
                subtask2.show
